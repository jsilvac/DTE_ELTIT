...
Begin VB.Form prestamosvigentes 
   BackColor       =   &H00FFC0C0&
   ...
   End
End
Attribute VB_Name = "prestamosvigentes"
...
Sub leerprestamosvigentes(mes, año, FILTRO)
    ...
    csql.sql = "select pc.empresa,pc.comprobante,pc.numerocuota,p.cantidadcuotas,pc.mes,pc.año,pc.monto,pc.rut,pc.mesrebajado,pc.añorebajado "
    ...
    If FILTRO = "Verdadero" Then
        csql.sql = csql.sql & " where pc.mes='" & mes & "' and  pc.año='" & año & "' and pc.mesrebajado='' "
    Else
        csql.sql = csql.sql & "where pc.mesrebajado='' "
    End If
...
Function leernombretrabajador(rut, Empresaconsulta) As String
    ...
    csql.sql = "select nombre,direccion,ciudad "
...
End Function
 ...
 Private Sub imprimir()
        Dim i As Long
        Call Titulos("LISTADO PRESTAMOS VIGENTES")
        Gridprestamos.AutoRedraw = False
        Gridprestamos.PageSetup.HeaderMargin = 1
...
Sub Titulos(titulo1)
    ...
    Gridprestamos.PageSetup.Orientation = cellPortrait
