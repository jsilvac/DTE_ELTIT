...
Begin VB.Form listadetallevendedores 
   BackColor       =   &H00FFC0C0&
   ...
   End
End
Attribute VB_Name = "listadetallevendedores"
...
codigovendedor = tcajera.text + lbldv.Caption
If codigovendedor <> "" Then
            'actualializa datos de ultimas compras
            Call Actualizaultimascompras(Mid(combolocal.text, 1, 2))
...
Private Sub CargaGrillaInforme(ByVal row As Integer, ByVal col As Integer)
        ...
        formatogrilla(1, 9) = "COSTO"
        formatogrilla(1, 10) = "UTILIDAD"
        formatogrilla(1, 11) = "% MARGEN"
...
Private Function resumenVentas(ByRef data As Adodc, ByRef impresion As Grid, ByVal TIPO As String, ByVal codLoc As String, ByVal fecha1 As String, ByVal fecha2 As String) As Long
    ...
    Dim totalesge(10, 20) As Double
    Dim s As Integer
    Dim tipocli As Integer
    ...
    End If
    End If
    Call LEErnotasdecredito(fecha1, fecha2, Mid(combolocal.text, 1, 2), codigovendedor)
    ...
    rubAux = rubro
    tabla = "SELECT dd.tipo, dd.numero , dd.fecha, dd.rut, mc.nombre,sum(if(dd.tipo='FV' or dd.tipo='BV',dd.total,0)),sum(if(dd.tipo='NB' or dd.tipo='NF',dd.total,0)),sum(if(dd.tipo='FV' or dd.tipo='BV',dd.total,0))-sum(if(dd.tipo='NB' or dd.tipo='NF',dd.total,0)), sum(dd.cantidad*mpf.ultimopreciocompra), sum(dd.cantidad), dd.vendedor,count(dd.numero),mc.constructora "
    tabla = tabla & "FROM sv_documento_detalle_" & codigoempresa & " as dd ,sv_documento_cabeza_" & codigoempresa & " as dc," & baseVentas & ".sv_maestroclientes AS mc, " + clientesistema + "gestion" + leerRubro(codigoempresa) + ".r_maestroproductos_precios_" + leerRubro(codigoempresa) + " as mpf "
    tabla = tabla & "where dd.rut = mc.rut AND mc.sucursal = '0' and dd.tipo=dc.tipo and dd.numero=dc.numero and dd.caja=dc.caja and dd.fecha=dc.fecha and mpf.codigo=dd.codigo and mpf.codigoprecio='01' and mpf.local='" + codigoempresa + " ' "
    tabla = tabla & "and dd.fecha BETWEEN '" & fecha1 & "' AND '" & fecha2 & "' and (dd.tipo='FV' or dd.tipo='BV' or dd.tipo='NB' or dd.tipo='NF') AND dd.local='" + codigoempresa + "' and dc.caja<'90' and dc.nula<>'S' "
    ...
    End If
     ...
     tabla = tabla & "group by dd.numero,dd.vendedor ORDER BY dd.vendedor,mc.rut,dd.numero "
    ...
    Else
        ...
        tabla = tabla & "sum(dd.descuento)/" + iva + " as descuento, "
...
''           impresion.Column(1).Locked = False
        ...
        FILTRO = resultados("vendedor")
           ...
           If VISTA1.Value = True Then
            ...
            If resultados(0) <> "NB" And resultados(0) <> "NF" Then
            impresion.Cell(linea, 10).text = resultados(7) - resultados(8)
            impresion.Cell(linea, 11).text = (resultados(7) - resultados(8)) / resultados(8) * 100
           ...
           End If
            ...
            If resultados(0) <> "NB" And resultados(0) <> "NF" Then
            totales(5) = totales(5) + CDbl(resultados(7) - CDbl(resultados(8)))
            ...
            totales2(5) = totales2(5) + CDbl(resultados(7) - CDbl(resultados(8)))
            totales2(6) = totales2(6) + CDbl(resultados(9))
            End If
            tipocli = Val(resultados("constructora"))
            
            totalesge(1, tipocli) = totalesge(1, tipocli) + CDbl(resultados(5))
            totalesge(2, tipocli) = totalesge(2, tipocli) + CDbl(resultados(6))
            totalesge(3, tipocli) = totalesge(3, tipocli) + CDbl(resultados(7))
            totalesge(4, tipocli) = totalesge(4, tipocli) + CDbl(resultados(8))
            If resultados(0) <> "NB" And resultados(0) <> "NF" Then
            
            totalesge(5, tipocli) = totalesge(5, tipocli) + CDbl(resultados(7) - CDbl(resultados(8)))
            totalesge(6, tipocli) = totalesge(6, tipocli) + CDbl(resultados(9))
    ...
    linea = linea + 1
           ...
           impresion.Cell(linea, 8).text = totales(3)
           impresion.Cell(linea, 9).text = totales(4)
           impresion.Cell(linea, 10).text = totales(3) - totales(4)
           impresion.Cell(linea, 11).text = (totales(3) - totales(4)) / totales(4) * 100
           impresion.Cell(linea, 12).text = ((totales(3) / totalventa) * 100)
    ...
    linea = linea + 1
           ...
           impresion.Cell(linea, 4).text = "TOTAL GRUPO " & s
            
          
            impresion.Cell(linea, 6).text = totalesge(1, s)
            impresion.Cell(linea, 7).text = totalesge(2, s)
            impresion.Cell(linea, 8).text = totalesge(3, s)
            impresion.Cell(linea, 9).text = totalesge(4, s)
            impresion.Cell(linea, 10).text = totalesge(3, s) - totalesge(4, s)
            impresion.Cell(linea, 11).text = ((totalesge(3, s) - totalesge(4, s)) / totalesge(4, s) * 100)
            impresion.Cell(linea, 12).text = ((totalesge(3, s) / totalventa) * 100)
...
impresion.Column(4).Locked = False
           ...
           impresion.Cell(linea, 10).text = totales2(3) - totales2(4)
           impresion.Cell(linea, 11).text = (totales2(3) - totales2(4)) / totales2(4) * 100
           impresion.Cell(linea, 12).text = ((totales2(3) / totalventa) * 100)
...
nombrecajera.Caption = leerNombreVendedor(tcajera.text + lbldv.Caption)
...
'    Grid1.ReportTitles.Add objReportTitle
    ...
    If TIPO2.Value = True Then
    Set objReportTitle = New FlexCell.ReportTitle
    objReportTitle.text = "VENDEDOR: " & tcajera.text
    objReportTitle.Font.Name = "Verdana" '"Times New Roman"
    objReportTitle.Font.Size = 8
    objReportTitle.Font.Bold = True
    objReportTitle.Align = cellCenter
...
Public Function LEErnotasdecredito(DESDE, HASTA, empresa, vendedor) As Double
    ...
    Dim csql As New rdoQuery
        ...
        csql.sql = csql.sql + "where fecha between '" + DESDE + "' and '" + HASTA + "' and (tipo='NB' or tipo='NF') and vendedor='" + vendedor + "' "
        csql.Execute
...
Public Function LEERCOSTOFACTURA(TIPO, NUMERO, fecha, caja, empresa) As Double
    ...
    Set csql.ActiveConnection = ventasRubro
        ...
        csql.sql = "select sum(dd.cantidad)*(mpf.pcosto) " + clientesistema + "ventas" + empresa + ".sv_documento_detalle_" + empresa + " left join " + clientesistema + leerRubro(empresa) + ".r_maetroproductos_fijo_" + leerRubro(empresa) + " as mpf on mpf.codigobarra=dd.codigo "
        csql.sql = csql.sql + "where tipo='" + TIPO + "' and numero='" + NUMERO + "' and fecha='" + Format(fecha, "yyyy-mm-dd") + "' and caja='" + caja + "'  "
        csql.Execute
        
End Function

Public Function Actualizaultimascompras(loca)
   Dim resultados As rdoResultset
   Dim csql As New rdoQuery
   Set csql.ActiveConnection = gestionRubro
   csql.sql = " UPDATE r_maestroproductos_precios_" & leerRubro(loca) & " mp JOIN "
   csql.sql = csql.sql & "(SELECT codigo,fecha,cantidad,precio, rut  FROM (SELECT * From l_movimientos_detalle_" & loca & " "
   csql.sql = csql.sql & "WHERE tipo = 'OC' ORDER BY fecha DESC) AS medio GROUP BY codigo) AS uno "
   csql.sql = csql.sql & "ON uno.codigo=mp.codigo AND mp.local='" & loca & "' SET mp.ultimopreciocompra = uno.precio, mp.ultimofechacompra =uno.fecha,mp.rutultimacompra=uno.rut"
   csql.Execute
End Function
