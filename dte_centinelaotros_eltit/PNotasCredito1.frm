...
Begin VB.Form PNotasCredito 
   ...
   Begin XPFrame.FrameXp frmDatos 
      ...
      Begin VB.ComboBox cmbTipo 
         ...
         BackColor       =   &H00E1FFFD&
         Height          =   315
         ItemData        =   "PNotasCredito.frx":0000
         Left            =   1920
         List            =   "PNotasCredito.frx":0007
...
'============================================================
    ...
    Private Sub dato11_KeyUp(KeyCode As Integer, Shift As Integer)
        ...
        If dato11.text <> "" And dato11.text <> "0" Then
            desc = CDbl(dato11.text) * 100 / total
            dato10.text = desc
            lblTotal.Caption = Round(total - CDbl(dato11.text), 0)
        Else
            dato10.text = "0"
...
'    End Sub
    ...
    Private Sub detalle_KeyDown(KeyCode As Integer, ByVal Shift As Integer)
        ...
        If detalle.ActiveCell.col = 1 And KeyCode = vbKeyF2 Then Call ayudaProducto(detalle, pivote)
        Select Case KeyCode
            Case 13, 37, 38, 39, 40
    ...
    Private Sub detalle_LeaveCell(ByVal row As Long, ByVal col As Long, NewRow As Long, NewCol As Long, Cancel As Boolean)
        ...
        If vacio = True Then
            ...
            End If
        Else
            If col = detalle.Cols - 1 And NewCol = 1 Then
            ...
            If lista.Cell(i, 1).text <> "$ 0" And lista.Cell(i, 1).text <> "" Then
                ...
                Select Case leerImpuestoProducto(codigo)
                    Case "IHA"
                        ...
                        totIVA = totIVA + totPro - totPro * iva / 100
                        totIHA = totIHA + totPro - totPro * iha / 100
                    Case "IVA"
                        totPro = CDbl(lista.Cell(i, 5).text)
                        totIVA = totIVA + totPro - totPro * iva / 100
                End Select
...
'=============================================================================
    Private Sub structtoctrlCliente()
        ...
        If lblDias.Caption <> "" Then
            fecha = DateAdd("d", lblDias.Caption, Date)
        Else
            fecha = Date
        End If
...
'=============================================================================
    ...
    Private Sub imprimir()
        Call imprimeNotaCredito(dato2.text, impresion, data)
    End Sub
    
    Private Sub retorno()
        Call LimpiarCajas(PVentas)
        Call LimpiarLabels(PVentas)
        Call cargaGrilla(1, 6)
        modifica = False
        Call DeshabilitarCajas(Me)
        cmbTipo.SetFocus
