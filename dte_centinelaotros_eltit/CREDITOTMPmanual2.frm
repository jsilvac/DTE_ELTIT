...
Private Sub abono_GotFocus()
Call selecciona(abono)
End Sub
...
If KeyAscii = 13 And abono.text <> "" Then
...
MONTO.text = Str(CDbl(VALORCUOTA.text) * CDbl(numerocuota.text))
...
'   If CDbl(lblCupo.Caption) >= CDbl(lblUtilizado.Caption) + CDbl(MONTO.text) Then
'      CALCULACUOTA
'   Else
...
Private Sub contado_Click()
CALCULAR_Click
End Sub

'Private Sub cuota_GotFocus()
'Call cargatexto(cuota)
'End Sub

'Private Sub cuota_KeyPress(KeyAscii As Integer)
'KeyAscii = esNumero(KeyAscii)
'If cuota.text <> "" Then
'If KeyAscii = 13 And cuota.text <> "" And cuota.text <> "0" And CDbl(cuota.text) <= leefactorMAXIMO Then
...
CARGAGRILLA
CARGAGRILLA2
Rem DATOSCREDITO.Enabled = False
Rem FRMCUOTAS.Enabled = False
Option2.Value = True
...
Dim cuota As Double
If contado.Value = 0 Then
FACTOR = leefactor(numerocuota.text)
cuota = Int((CDbl(Replace(MONTO.text, ",", "")) * FACTOR / CDbl(numerocuota.text)) + 0.5)
Else
cuota = Int((CDbl(Replace(MONTO.text, ",", "")) / CDbl(numerocuota.text)) + 0.5)
End If
...
NUMERO.text = Folioingresomanual
abono.text = "0"
...
Public Function LEERDOCUMENTO(TIPO, NUMERO) As Boolean
        ...
        MONTO.text = resultado(1) - resultado(7)
...
If LEErcuotas(TIPO.text, NUMERO.text) = True Then
...
If KeyAscii = 13 Then
If TIPO.text <> "BV" And TIPO.text <> "FV" And TIPO.text <> "ZE" And TIPO.text <> "IM" Then
TIPO.SetFocus
Else
NUMERO.SetFocus
End If
...
End Sub
 ...
 Public Sub grabarcuotas()
        ...
        CAMPOS(9, 1) = CDbl(MONTO.text) / CDbl(numerocuota.text)
        CAMPOS(10, 1) = Replace(MONTO.text, ".", "")
...
Public Sub IMPRIMEcredito()
    ...
    Print #numfic,
    Print #numfic, "MONTO VENTA :"; MONTO.text
    
    Print #numfic, "SEGUN CONTRATO MAAT AUTORIZO CARGAR A MI "
    Print #numfic, "CUENTA "
    
    Print #numfic, numerocuota.text + " CUOTAS de " + VALORCUOTA.text + " Pesos"
