...
Begin VB.Form DocumentosEmitidos 
   BackColor       =   &H00FFC0C0&
   ...
   End
End
Attribute VB_Name = "DocumentosEmitidos"
...
Private Sub CargaGrillaInforme(ByVal row As Integer, ByVal col As Integer)
        ...
        formatoGrilla(1, 6) = "I.V.A"
        formatoGrilla(1, 7) = "EXENTO"
        formatoGrilla(1, 8) = "TOTAL"
        formatoGrilla(1, 9) = "DONACION"
'        formatoGrilla(1, 8) = "TOTAL"
...
Public Sub generaInformeLV(ByRef data As Adodc, ByRef impresion As Grid, ByVal TIPO As String, ByVal detalle As Boolean, ByVal codLoc As String, ByVal fecha1 As String, ByVal fecha2 As String)
   ...
   Call cargaCabeza("LISTADO DOCUMENTOS " & Format(fecha1, "dd-mm-yyyy") & " HASTA " & Format(fecha2, "dd-mm-yyyy"), empresaActiva, impresion)
...
'    impresion.Cols = 8
   ...
   If codigoempresa <> "99" Then
    tabla = "SELECT  dc.tipo,dc.foliosii,dc.rut,dc.fecha,dc.neto,dc.iva,dc.exento,dc.total,DONACION,dc.local "
    tabla = tabla & "FROM sv_documento_cabeza_" + codigoempresa + " AS dc "

    tabla = tabla & "WHERE fecha BETWEEN '" & fecha1 & "' AND '" & fecha2 & "' and  " & TIPO & "   and caja <'90' and dc.local='" + codigoempresa + "' "
    tabla = tabla & "order by dc.foliosii asc"
       ...
       impresion.Rows = 2
        While Not data.Recordset.EOF
           ...
           linea = linea + 1
            ...
            impresion.Cell(linea, 3).text = data.Recordset.Fields(3)
         
            impresion.Cell(linea, 4).text = leerNombreCliente(data.Recordset.Fields(2))
            ...
            totales(1) = totales(1) + CDbl(data.Recordset.Fields(4))
            totales(2) = totales(2) + CDbl(data.Recordset.Fields(5))
            totales(5) = totales(5) + CDbl(data.Recordset.Fields(6))
...
'            impresion.Cell(linea, 9).text = totales(4)
        ...
        totales2(3) = totales(3)
        totales2(4) = totales(4)
        totales2(5) = totales(5)
         For i = 1 To 4
        totales(i) = 0
        Next i
           ...
           impresion.Cell(linea, 1).text = "TOTAL GENERAL "
            
            impresion.Cell(linea, 5).text = totales2(1)
            impresion.Cell(linea, 6).text = totales2(2)
            impresion.Cell(linea, 7).text = totales2(5)
...
'
    ...
    If Option1.Value = True And Mid(combolocal.List(Q), 1, 2) <> "42" And Mid(combolocal.List(Q), 1, 2) <> "43" Then GoTo pasa:
    If Option2.Value = True And (Mid(combolocal.List(Q), 1, 2) = "42" Or Mid(combolocal.List(Q), 1, 2) = "43") Then GoTo pasa:
    
    
    tabla = "SELECT  dc.tipo,dc.foliosii,dc.rut,dc.fecha,dc.neto,dc.iva,dc.exento,dc.total,DONACION,dc.local "
    tabla = tabla & "FROM sv_documento_cabeza_" + Mid(combolocal.List(Q), 1, 2) + " AS dc "
    tabla = tabla & "WHERE fecha BETWEEN '" & fecha1 & "' AND '" & fecha2 & "' and " & TIPO & "  and caja <'90' and dc.local='" + Mid(combolocal.List(Q), 1, 2) + "' "
    tabla = tabla & "order by dc.foliosii asc"
    ...
    ' total vendedor
        ...
        totales2(3) = totales2(3) + totales(3)
        totales2(4) = totales2(4) + totales(4)
        totales2(5) = totales2(5) + totales(5)
...
'             impresion.Rows = impresion.Rows + 1
           ...
           impresion.Range(linea, 1, linea, 9).Borders(cellEdgeBottom) = cellThin
           impresion.Range(linea, 1, linea, 9).Borders(cellEdgeRight) = cellThin
           impresion.Range(linea, 1, linea, 4).Merge
           impresion.Cell(linea, 1).text = "TOTAL LOCAL " & leerNombreEmpresa(Mid(combolocal.List(Q), 1, 2))
            
            impresion.Cell(linea, 5).text = totales(1)
            impresion.Cell(linea, 6).text = totales(2)
            impresion.Cell(linea, 7).text = totales(5)
            impresion.Cell(linea, 8).text = totales(3)
            impresion.Cell(linea, 9).text = totales(4)
             linea = linea + 1
             impresion.Rows = impresion.Rows + 1
