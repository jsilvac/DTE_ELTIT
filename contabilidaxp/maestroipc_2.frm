...
Private Sub comboaño_Click()
    Call leeripc(COMBOAÑO.text)
End Sub

Private Sub COMMAND2_Click()
'Shell "iexplore http://www.sii.cl/pagina/valores/correccion/correccion" & Val(COMBOAÑO) & ".htm"
End Sub
...
Private Sub Form_Load()
    ...
    For k = Val(Format(fechasistema, "yyyy")) - 5 To Val(Format(fechasistema, "yyyy"))
        COMBOAÑO.AddItem k
        COMBOAÑO.text = COMBOAÑO.List(COMBOAÑO.ListCount - 1)
    Next k
    COMBOAÑO.Refresh
End Sub
Sub leeripc(año)
...
csql.sql = "select mes,porcentaje from ipc_nuevo where año='" & año & "' order by mes"
csql.Execute
Grid1.Rows = 14
...
Private Sub Command1_Click()
 ...
 For k = 1 To Grid1.Rows - 1
    i = k
    With Grid1
        Call grabar(i, COMBOAÑO.text, .Cell(k, 2).text, .Cell(k, 3).text)
    End With
    Next k
leeripc (COMBOAÑO.text)
End Sub
...
Sub grabar(ByRef dato0, dato1, dato2, dato3)
    ...
    campos(0, 2) = "ipc_nuevo"
    opcion = 4
    condicion = "año='" & COMBOAÑO.text & "'" & " and mes = '" & Format(dato0, "00") & "'"
