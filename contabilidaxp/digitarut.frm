...
Begin VB.Form digitarut 
   ...
   Begin XPFrame.FrameXp frmrut 
      ...
      End
      Begin VB.Label tipo 
         Caption         =   "VV tipo de Comprobante que viene"
         Height          =   255
         Left            =   3960
         TabIndex        =   9
         Top             =   840
...
Public Sub DATO20_KeyPress(KeyAscii As Integer)
    ...
    ' DESDE AQUI*******************************
    'ARIEL EVITA PAGO A PROVEEDOR CON ATICIPO
    If LEERSALDOSCTACTEmovi(leerdatos(conta, "maestroempresas", "ctaanticipoproveedor", "codigoempresa='" + empresaactiva + "' "), DATO20.text + DV.Caption, empresaactiva) > 0 And (Mid(tipo.Caption, 1, 2) = "CE" Or Mid(tipo.Caption, 1, 2) = "PA") Then
        If Verifica_Permiso("ingreso de comprobantes contables", "autoriza") = True Then
            If MsgBox("PROVEEDOR CON ANTICIPO SEGURO DESEA PAGAR", vbOKCancel, "ADVERTENCIA") = vbOK Then
                On Error GoTo otro:
                RUTOK.SetFocus
otro:
            Else
            DATO20.text = ""
            nombrectacte.Caption = ""
            DATO20.SetFocus
         End If
     Else
        If mensaje_nopermiso = "Periodo Cerrado. Imposible Hacer Modificaciones" Then
            MsgBox mensaje_nopermiso, vbCritical, "ATENCION"
             DATO20.text = ""
            nombrectacte.Caption = ""
            DATO20.SetFocus
        Else
            MsgBox "Esta cuenta Tiene Un Anticipo y ud no cuenta con el acceso para hacer este pago"
            DATO20.text = ""
            nombrectacte.Caption = ""
            DATO20.SetFocus
        End If
   ...
   If Me.Visible = True Then RUTOK.SetFocus
    Else
        'ARIEL EVITA QUE SE DEPLEGUE EL MAESTRO02 SI EL USUARIO NO LO DESEA
        'O SE EQUIVOCA DE RUT
        If MsgBox("CTA NO EXISTE DESEA CREAR?", vbOKCancel, "ADVERTENCIA") = vbOK Then
        maestro02.dato1.text = lblcuenta.Caption
        maestro02.dato2.text = DATO20.text
        maestro02.DV.Caption = DV.Caption   '
        If maestro02.Visible = False Then
        
        maestro02.Show 1
        Else
        MsgBox "ya se encuentra el formulario abierto "
