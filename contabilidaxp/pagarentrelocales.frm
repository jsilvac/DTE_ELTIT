...
For k = 1 To Grid1.Rows - 1
    If Grid1.Cell(k, 14).text = "1" Then
        ...
        TOTALCheque = 0
        End If
                Call GRABARCOMPROBANTE(Grid1.Cell(k, 2).text, Grid1.Cell(k, 9).text, Grid1.Cell(k, 10).text, Format(Grid1.Cell(k, 15).text, "yyyy-mm-dd"), Grid1.Cell(k, 1).text, Grid1.Cell(k, 16).text, Grid1.Cell(k, 1).text, Grid1.Cell(k, 5).text, Grid1.Cell(k, 0).text)
...
Dim resultados As rdoResultset
    ...
    fecha2 = año + "-" + MES + "-" + "31"
        ...
        csql.sql = "SELECT mc.tipo,mc.numero,mc.fecha,mc.monto,mc.localorigen,autorizacancelacion,fechaautorizacionpago "
        csql.sql = csql.sql + "FROM l_movimientos_cabeza_" + Mid(ComboLOCAL.text, 1, 2) + " as mc "
        csql.sql = csql.sql + " where (mc.tipo='RL' or mc.tipo='RF' or mc.tipo='RG') AND mc.fecha between '" + fecha1 + "' AND '" + fecha2 + "' "
        csql.sql = csql.sql + " order by mc.numero "
        csql.Execute
        total = 0
        total2 = 0
        Grid1.Rows = 1
        ...
        RUTEMPRE = leerdatoslocal(localfiltro, "rut")
...
'            Print "hola"
'            End If
            rutreal = leerdatoslocal(resultados(4), "rut")
            pagada = False
            
            If resultados(0) = "RF" Then
            numdoc = leefoliofiscal(resultados(4), "RF", resultados(1))
            montofacturas = leemontofactura("FAE", numdoc, rutreal)
            End If
            If resultados(0) = "RL" Then
            montofacturas = leemontofactura("FA", resultados(1), rutreal)
            numdoc = resultados(1)
            End If
            
            Rem If numdoc = "0000147358" Then Stop
            'montofacturas = leemontofactura("FA", resultados(1), rutreal)
            Rem If montofacturas = 0 Then Stop
            If pagada = False Then
            If rutreal <> RUTEMPRE Then
            
             Grid1.Rows = Grid1.Rows + 1
             LINEA = LINEA + 1
             Grid1.Cell(LINEA, 0).text = resultados(4)
             Grid1.Cell(LINEA, 1).text = resultados(0)
             Rem Grid1.Cell(linea, 1).text = "CHEQUES"
             Grid1.Cell(LINEA, 2).text = numdoc
             ...
             Grid1.Cell(LINEA, 5).text = resultados(2)
             Grid1.Cell(LINEA, 6).text = montofacturas
             Grid1.Cell(LINEA, 7).text = montofacturas
             If montofacturas = 0 Then Grid1.Range(LINEA, 1, LINEA, 10).BackColor = vbRed
             OTROS = 0
             Grid1.Cell(LINEA, 8).text = OTROS
             
             apagar = montofacturas
             DIFERENCIA = 0
            ...
            Dim TIPOREVISA As String
            
             If resultados(0) = "RF" Then TIPOREVISA = "FAE" Else TIPOREVISA = "FA"
             If leefactura(TIPOREVISA, numdoc, rutreal) = False Then
               
               contabilizada = "0"
               Else
               contabilizada = "1"
               End If
             Grid1.Cell(LINEA, 16).text = resultados(1)
               
             Grid1.Cell(LINEA, 13).text = contabilizada
             Grid1.Cell(LINEA, 14).text = resultados(5)
...
Call MsgBox("debe rebajar devoluciones antes cancelar5")
...
Call modificaordenpagoentre(Grid1.Cell(fila, 16).text, localfiltro, Grid1.Cell(fila, 14).text, Format(Grid1.Cell(fila, 15).text, "yyyy-mm-dd"), Grid1.Cell(fila, 1).text, Grid1.Cell(fila, 5).text)
End If
...
Public Function leefactura(tipo, numero, rut) As Boolean
    ...
    campos(3, 0) = "abono"
    campos(4, 0) = ""
    If tipo = "FA" Then TIPODOCU = "1"
    If tipo = "ND" Then TIPODOCU = "2"
    If tipo = "NC" Then TIPODOCU = "3"
    If tipo = "FAE" Then TIPODOCU = "4"
    If tipo = "NDE" Then TIPODOCU = "5"
    If tipo = "NCE" Then TIPODOCU = "6"
    
    condicion = "tipo='" + TIPODOCU + "' and numero='" + numero + "' and rut='" + rut + "' "
...
End Function
Sub GRABARCOMPROBANTE(ORDEN, montocheque, DIFERENCIA, fechacheque, TIPOPAGO, glosadiferencia, tipodocumento, fechadocumento, LOCALORIGEN)
    Dim DH As String
    Dim numero As String
    Dim LINEA As Double
    Dim fecha As Date
    Dim rut As String
    Dim tipodocumento2 As String
    Dim numerodocumento As String
    Dim fechadocumento2 As String
    ...
    If tipodocumento = "RF" Then
    numerointerno = leefoliointerno(LOCALORIGEN, "33", CDbl(ORDEN))
    End If
    
    
        csql.sql = "SELECT 'FA',localorigen,monto,numero,fecha "
        csql.sql = csql.sql + "FROM l_movimientos_cabeza_" + localfiltro + " WHERE numero='" + numerointerno + "' and (tipo='RL' or tipo='RF') and fecha='" + Format(fechadocumento, "yyyy-mm-dd") + "' "
        ...
        If csql.RowsAffected > 0 Then
            ...
            rut = resultados(2)
            tipodocumento2 = resultados(0)
            If tipodocumento = "RF" Then
            tipodocumento2 = "FAE"
            End If
            
            TIPO3 = resultados(0)
            If tipodocumento2 = "FA" Then tipo2 = "1": TIPO3 = "FC"
            If tipodocumento2 = "ND" Then tipo2 = "2": TIPO3 = "ND"
            If tipodocumento2 = "NC" Then tipo2 = "3": TIPO3 = "NC"
            If tipodocumento2 = "FAE" Then tipo2 = "4": TIPO3 = "FC"
            If tipodocumento2 = "NDE" Then tipo2 = "5": TIPO3 = "ND"
            If tipodocumento2 = "NCE" Then tipo2 = "6": TIPO3 = "NC"
            If Mid(TIPOPAGO, 1, 2) = "RL" Or Mid(TIPOPAGO, 1, 2) = "RF" Then
            DOCUMENTOPAGO = "PA"
            Else
            DOCUMENTOPAGO = "DB"
            End If
            ...
            rut2 = leerdatoslocal(resultados(1), "rut")
            Call grabarcomprobante_lineas(DOCUMENTOPAGO, numerocontable, lineacontable, fecha, CUENTAPROVEEDOR, " ", rut2, " ", "CANCELA DOCUMENTO", tipodocumento, ORDEN, fechadocumento, fechavencimiento, monto, DH, USUARIOSISTEMA, MES, año, Format(Date, "yyyy-mm-dd"), Time, rutreal)
            
            montofactura2 = leemontofactura(tipodocumento2, ORDEN, rut2)

            Call abonofactura(tipo2, ORDEN, rut2, montofactura2)
   ...
   Rem graba linea  diferencia
        ...
        Rem Call leerotros(DOCUMENTOPAGO, numerocontable, fecha, localfiltro, rut, ORDEN, lineacontable, MES, año)
...
Public Sub leerotros(tipo, numero, fecha, loc, rut, ORDEN, LINEA, MES, año)
        ...
        tabla = tabla & "FROM l_ordendecompra_anexopagos_" + loc + " "
...
Rem Call modificaordenpagoentre(Grid1.Cell(row, 2).text, localfiltro, Grid1.Cell(row, 14).text, Grid1.Cell(row, 15).text, Grid1.Cell(row, 16).text)
fila = row
Call modificaordenpagoentre(Grid1.Cell(fila, 2).text, localfiltro, Grid1.Cell(fila, 14).text, Format(Grid1.Cell(fila, 15).text, "yyyy-mm-dd"), Grid1.Cell(fila, 1).text, Grid1.Cell(fila, 5).text)
End If
...
Public Function leefoliofiscal(loca, tipo, numero) As String
    Dim resultados As rdoResultset
    Dim csql As New rdoQuery
        Set csql.ActiveConnection = contadb
            csql.sql = "select numero from eltit_fae" & loca & ".sv_dte" & loca & " where tipo = '33' AND cajadocumento='99' and numerodocumento = '" & numero & "'"
            csql.Execute
    If csql.RowsAffected > 0 Then
    Set resultados = csql.OpenResultset
        leefoliofiscal = Format(resultados(0), "0000000000")
    End If
End Function

Public Function leefoliointerno(loca, tipo, numero) As String
    Dim resultados As rdoResultset
    Dim csql As New rdoQuery
        Set csql.ActiveConnection = contadb
            csql.sql = "select numerodocumento from eltit_fae" & loca & ".sv_dte" & loca & " where tipo = '33' AND numero = '" & numero & "'"
            csql.Execute
    If csql.RowsAffected > 0 Then
    Set resultados = csql.OpenResultset
        leefoliointerno = Format(resultados(0), "0000000000")
    End If
End Function
