...
Begin VB.Form proceso08 
   ...
   Begin XPFrame.FrameXp FrameXp1 
      ...
      _ExtentY        =   18256
      BackColor       =   12632256
      Caption         =   "CENTRALIZACION DE MONEDAS EXTRANJERAS"
      CaptionEstilo3D =   1
      BackColor       =   12632256
    ...
    Dim k As Double
        For k = 1 To Grid1.Rows - 1
            If Grid1.Cell(k, 9).text <> "1" Then
                If Grid1.Cell(k, 3).text = "000000001-9" Then
                    glosa = "COMPRA US " & Format(Grid1.Cell(k, 6).text, "###,###,##0") & " X " & Format(Grid1.Cell(k, 7).text, "###,###,##0")
                Else
                 glosa = "COMPRA € " & Format(Grid1.Cell(k, 6).text, "###,###,##0") & " X " & Format(Grid1.Cell(k, 7).text, "###,###,##0")
                End If
                Call grabafactura(LEERULTIMOFOLIO(Grid1.Cell(k, 1).text), Format(Grid1.Cell(k, 5).text, "yyyy-mm-dd"), Grid1.Cell(k, 3).text, Grid1.Cell(k, 8).text, Grid1.Cell(k, 1).text, glosa)
            End If
        Next k
    leer
End Sub
...
Rem DATOS DE LA COLUMNA
    ...
    FORMATOGRILLA(1, 6) = "CANTIDAD"
    FORMATOGRILLA(1, 7) = "MONTO CAMBIO"
    FORMATOGRILLA(1, 8) = "TOTAL"
    FORMATOGRILLA(1, 9) = "CONTA"
    FORMATOGRILLA(1, 10) = "CUENTA"
...
Dim resultados As rdoResultset
    ...
    fecha2 = Format(hastafecha.Caption, "yyyy-mm-dd")
        ...
        csql.sql = "SELECT 'CM',folio,IF(tipomoneda='01','0000000019','0000000035') AS rut,"
        csql.sql = csql.sql & "IF(tipomoneda='01','DOLARES','EUROS') AS nombre,fecha,SUM(cantidad),tipocambio,"
        csql.sql = csql.sql & "SUM(cantidad)* tipocambio AS total FROM cambio_dolares WHERE fecha "
        csql.sql = csql.sql & "BETWEEN '" & fecha1 & "' AND '" & fecha2 & "' GROUP BY fecha,tipocambio "
        ...
        If csql.RowsAffected > 0 Then
         ...
         While Not resultados.EOF
             ...
             Grid1.Cell(LINEA, 5).text = Format(resultados(4), "dd-mm-yyyy")
             Grid1.Cell(LINEA, 6).text = resultados(5)
             Grid1.Cell(LINEA, 7).text = resultados(6)
              Grid1.Cell(LINEA, 8).text = resultados(7)
            If estacontabilizadoCM(resultados(0), resultados(4)) = True Then
                Grid1.Cell(LINEA, 9).text = "1"
            End If
...
End Sub
Function estacontabilizadoCM(tipo, fecha) As Boolean
    Dim csql As New rdoQuery
    Set csql.ActiveConnection = contadb
    csql.sql = "select numero from movimientoscontables where tipo='" & tipo & "' and fecha='" & Format(fecha, "yyyy-mm-dd") & "' "
    csql.Execute
        estacontabilizadoCM = False
    If csql.RowsAffected > 0 Then
            estacontabilizadoCM = True
    End If
...
Sub grabafactura(numero, fecha, rut, monto, TIPOTR, glosa)
    Dim netos As Double
    Dim DH As String
    ...
    Call grabardetallepagos(numero, rut, numero, TIPOTR, monto, fecha, glosa)
End Sub
...
Sub grabardetallepagos(numero, rut, FOLIO, TIPOTR, monto, fecha, glosa)
    Dim resultados As rdoResultset
    Dim csql As New rdoQuery
    ...
    Dim cuentacontable As String
            ...
            cuentacontable = "11100005"
            rut = Replace(rut, "-", "")
            Call grabarcomprobante_lineas(TIPODO, numero, Format(1, "000"), fecha, cuentacontable, "", rut, "", glosa, TIPODO, numerodo, fechado, fechado, monto, "D", USUARIOSISTEMA, Format(fecha, "MM"), Format(fecha, "YYYY"), Format(fechasistema, "yyyy-mm-dd"), Time, "")
            
           
            numerodo = numero
            fechado = fecha
            glosa = "COMPRA MONEDA EXTRANJERA " & Format(fecha, "dd-mm-yyyy")
            cuentacontable = "11100006"
            Call grabarcomprobante_lineas(TIPODO, numero, Format(2, "000"), fecha, cuentacontable, "", "0000000019", "", glosa, TIPODO, numerodo, fechado, fechado, monto, "H", USUARIOSISTEMA, Format(fecha, "MM"), Format(fecha, "YYYY"), Format(fechasistema, "yyyy-mm-dd"), Time, "")
...
Sub leectacte(rut)
    ...
    condicion = "tipo=" + "'11100005' and rut=" + "'" + rut + "' and año='" + año + "'"
    op = 5
    sqlconta.response = campos
    Set sqlconta.conexion = contadb
    Call sqlconta.sqlconta(op, condicion)
