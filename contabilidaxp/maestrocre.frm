...
Sub carga()
    ...
    dato15.text = Format(sqlconta.response(10, 3), "###,###,###,##0")
    ...
    TOTA5 = TOTA2 - TOTA4
    CAPITAL.text = Format(TOTA1, "###,###,###,##0")
    PROYECTADO.text = Format(TOTA2, "###,###,###,##0")
    SOBRE.text = Format(TOTA3, "###,###,###,##0")
    CANCELADO.text = Format(TOTA4, "###,###,###,##0")
    SALDO.text = Format(TOTA5, "###,###,###,##0")
...
Public Sub leermensualidades(banco, tipo, numero, empresa)
 ...
 Set csql.ActiveConnection = db
 csql.sql = "select fecha,monto,pagado from " & clientesistema & "creditos_bancarios" & ".creditos_vencimientos as mp where tipo='" + tipo + "' and banco='" + banco + "' and empresa='" + empresa + "' and numero='" + numero + "' "
