...
Begin VB.Form banco01 
   Appearance      =   0  'Flat
   BackColor       =   &H00FFC0C0&
   BorderStyle     =   1  'Fixed Single
   Caption         =   "Rebaja Cheques de Cartola"
   ClientHeight    =   9120
   ClientLeft      =   45
   ClientTop       =   435
   ClientWidth     =   10410
   ...
   Begin XPFrame.FrameXp FrameXp1 
      ...
      EndProperty
      Begin VB.CommandButton CADUCADO 
         Caption         =   "Caducar"
         Height          =   285
         Left            =   8550
         TabIndex        =   36
      ...
      Begin VB.CommandButton anular 
         ...
         Width           =   1185
      End
      Begin VB.CommandButton liberar 
         Caption         =   "Liberar"
         Height          =   285
         Left            =   8550
         TabIndex        =   29
...
Sub leercheque()
    ...
    If sqlconta.response(10, 3) <> "0" Then
    If IsNull(sqlconta.response(8, 3)) = False Then
    fechacobro.Caption = Mid(sqlconta.response(8, 3), 1, 2) + "-" + Mid(sqlconta.response(8, 3), 4, 2) + "-" + Mid(sqlconta.response(8, 3), 7, 4)
    End If
    If sqlconta.response(10, 3) = "1" Then estado.Caption = "COBRADO AUTOMATICO"
    If sqlconta.response(10, 3) = "2" Then estado.Caption = "COBRADO MANUAL"
    If sqlconta.response(10, 3) = "3" Then estado.Caption = "CADUCADO"
    If sqlconta.response(10, 3) = "4" Then estado.Caption = "O.N.P"
    If sqlconta.response(10, 3) = "5" Then estado.Caption = "ANULADO"
    
    End If
    tipo = sqlconta.response(9, 3)
    If tipo = "" Then tipo = "0"
    lbldis.Caption = GRILLATIPO.TextMatrix(CDbl(tipo), 1)
