...
'    End If
'
ARCHIVO = RUTACOMPROBANTE & "comprobantedepago.htm"
    If oprut.Value = False Then
    ...
    Grid3.ExportToHTML (RUTACOMPROBANTE & "comprobantedepago.htm")
    
     If ExisteArchivo(ARCHIVO) = False Then
         MsgBox "NO SE A GENERADO ARCHIVO ADJUNTO, POR FAVOR VERIFICAR EN " & ARCHIVO & " ", vbCritical, "ATENCION"
         Exit Sub
    End If
    
    
    Rem original Call EnviarMail2(ServerCorreo, UsuarioCorreo, ClaveCorreo, 25, nombreempresa, Grid3.Cell(Grid3.Rows - 4, 4).text, "rauls@eltit.cl", "", "CANCELACION DE FACTURAS", MENSAJE, ARCHIVO)
...
'   Call enviaremail2(ServerCorreo, UsuarioCorreo, ClaveCorreo, 25, nombreempresa, "cesarsandoval@adminerp.cl;sarcesnot@hotmail.com", "rauls@eltit.cl", "", "CANCELACION DE FACTURAS", MENSAJE, ARCHIVO)
...
'   Grid3.Cell(Grid3.Rows - 4, 4).text = "sarcesnot@hotmail.com"
...
''destinatario = "cesarsandoval@adminerp.cl"
'ssl = 1
'puertosalida = 25
'If InStr(servidor_correo, "eltit") > 0 Then
'    ssl = 2
'    puertosalida = 465
'End If
'
...
Dim resultados As rdoResultset
    ...
    fecha2 = "2100-12-01"
        ...
        csql.sql = csql.sql + "where fechavencimiento>='" + fecha1 + "' AND fechavencimiento<='" + fecha2 + "' and tipo='DB' and codigocuenta='11130001' "
...
Public Sub LEERNOMINA(fecha)
    ...
    Dim csql As New rdoQuery
        ...
        Set csql.ActiveConnection = contadb
            csql.sql = "select mc.numero,mc.rutproveedor,cc.nombre, sum(mc.monto),mc.mes,mc.año from movimientoscontables as mc,cuentascorrientes as cc "
            csql.sql = csql.sql & "Where mc.codigocuenta='11130001' and cc.año='" + Mid(COMBOAÑO.text, 1, 4) + "' and mc.rutproveedor=cc.rut and cc.tipo='" + CUENTAPROVEEDOR + "' and fechavencimiento='" & Format(fecha, "yyyy-mm-dd") & "' and mc.tipo='DB' group by mc.rutproveedor order by cc.nombre "
            csql.Execute
...
Public Sub LEERNOMINA2(fecha)
    ...
    GENERANOMINA = True
        ...
        Set csql.ActiveConnection = contadb
            csql.sql = "select mc.numero,mc.rutproveedor,cc.nombre, mc.monto,mc.mes,mc.año from movimientoscontables as mc,cuentascorrientes as cc "
            csql.sql = csql.sql & "Where mc.codigocuenta='11130001' and mc.rutproveedor=cc.rut and cc.tipo='" + CUENTAPROVEEDOR + "' and fechavencimiento='" & Format(fecha, "yyyy-mm-dd") & "' and mc.tipo='DB' group by mc.rutproveedor,mc.numero order by cc.nombre "
            csql.Execute
   ...
   lineacom = 0
           End If
            ...
            Call LEERdetalle(resultados(0), resultados(4), resultados(5))
            
            
           
           resultados.MoveNext
           If resultados.EOF = False Then
            LINEAS = LINEAS + cuentalineas(resultados(0), resultados(4), resultados(5))
            If LINEAS > 11 Then
            revisanumero = True
            Else
            revisanumero = False
...
Public Sub LEERdetalle(numero, MES, año)
    ...
    Dim csql As New rdoQuery
        ...
        Set csql.ActiveConnection = contadb
            ...
            csql.sql = csql.sql & "Where mc.tipo='DB' and mc.numero='" + numero + "' and mes='" + MES + "' and año='" + año + "'  order by linea "
            csql.Execute
...
End Sub
Function cuentalineas(numero, MES, año) As Double
    ...
    Dim csql As New rdoQuery
        ...
        Set csql.ActiveConnection = contadb
            ...
            csql.sql = "select mc.numero,mc.rutproveedor,mc.glosacontable,mc.tipodocumento,mc.numerodocumento,if (mc.dh='D',mc.monto,mc.monto*-1),codigocuenta from movimientoscontables as mc "
            csql.sql = csql.sql & "Where mc.tipo='DB' and mc.numero='" + numero + "' and mes='" + MES + "' and año='" + año + "' and mc.codigocuenta<>'11130001'  order by linea "
            csql.Execute
    cuentalineas = 0
    If csql.RowsAffected > 0 Then
