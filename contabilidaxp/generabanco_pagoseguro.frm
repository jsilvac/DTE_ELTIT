...
Begin VB.Form generabancopagoseguro 
   BackColor       =   &H00FFC0C0&
   ...
   End
End
Attribute VB_Name = "generabancopagoseguro"
...
'        End If
 If GridLiquida.Cell(k, 9).text = "1" Then
        ...
        numerocomprobante = GridLiquida.Cell(k, 0).text
        rutprove = Mid(GridLiquida.Cell(k, 1).text, 1, 9) + Mid(GridLiquida.Cell(k, 1).text, 10, 1)
        rutcontable = rutprove
        'a nomina bancarias
'        CUENTABANCO = "47150022"
        CUENTABANCO = "11130002"
        fechavencimiento = Format(fechasistema, "yyyy-mm-dd")
        monto = CDbl(GridLiquida.Cell(k, 8).text)
        DH = "D"
        lineacontable = lineacontable + 1
        
'        If lineacontable > 1 Then GoTo SIGUIENTE:
'  If verificasiexiste2(rutcontable, CUENTABANCO, FECHACONTABLE, tipocontable, "CANC. SEGURO COMPLEMENTARIO ") = False Then
'
'SIGUIENTE:
            Call grabarcomprobante_lineas(tipocontable, numerocontable, lineacontable, _
            FECHACONTABLE, CUENTABANCO, " ", rutcontable, " ", "CANC. SEGURO COMPLEMENTARIO " & NOMBREGIRADO, _
            "PS", numerocomprobante, FECHACONTABLE, fechavencimiento, monto, DH, USUARIOSISTEMA, _
...
Sub CargaGridLiquida(grilla As Grid)
    ...
    If csql.RowsAffected > 0 Then
        ...
        While Not resultados.EOF
            LIQUIDO = leemontoSEGURO(resultados(0), Format(fechasistema, "mm"), Format(fechasistema, "YYYY"))
'             LIQUIDO = leeranticipos(resultados(0), Format(fechasistema, "mm"), Format(fechasistema, "YYYY"), "01000")
            'CONVENIO = totalconvenios(resultados(0))
            saldo = LIQUIDO - CONVENIO
            If saldo > 0 Then
             grilla.Rows = grilla.Rows + 1
            LINEA = grilla.Rows - 1
            grilla.Cell(LINEA, 0).text = leenumeroSEGURO(resultados(0), Format(fechasistema, "mm"), Format(fechasistema, "YYYY"))
...
'            Call calculaliquidaciones.calculaliquidaciones(resultados(0), Format(fechasistema, "mm"), Format(fechasistema, "YYYY"), empresaactiva)
    ...
    grilla.Cell(grilla.Rows - 1, 2).text = "TOTAL SEGURO COMPLEMENTARIO A PAGAR X BANCO "
    ...
    grilla.Cell(grilla.Rows - 1, 2).text = "TOTAL SEGURO COMPLEMENTARIO A PAGAR DIRECTOS "
    ...
    grilla.Refresh
End Sub
Function leemontoSEGURO(rut, MES, año) As Double
        Dim csql As New rdoQuery
    Dim resultados As rdoResultset
    
    Set csql.ActiveConnection = contadb
   
     csql.sql = "select sum(monto) from " & clientesistema & "conta" & empresaactiva & ".movimientoscontables "
     csql.sql = csql.sql & "where rutctacte ='" & rut & "' and mes='" & MES & "' and año='" & año & "' and "
     csql.sql = csql.sql & "tipo='PS' and (codigocuenta='47150022') and fechavencimiento<>'2100-01-01' group by rutctacte "
     csql.Execute
     
    If csql.RowsAffected > 0 Then
        Set resultados = csql.OpenResultset
        leemontoSEGURO = resultados(0)
        Else
        leemontoSEGURO = 0
    End If
    csql.Close
...
Function leenumeroSEGURO(rut, MES, año) As String
        Dim csql As New rdoQuery
    Dim resultados As rdoResultset
    
    Set csql.ActiveConnection = contadb
   
     csql.sql = "select numero from " & clientesistema & "conta" & empresaactiva & ".movimientoscontables "
     csql.sql = csql.sql & "where rutctacte ='" & rut & "' and mes='" & MES & "' and año='" & año & "' and "
     csql.sql = csql.sql & "tipo='PS' and (codigocuenta='47150022') and fechavencimiento<>'0000-00-00' group by rutctacte "
     csql.Execute
     
    If csql.RowsAffected > 0 Then
        Set resultados = csql.OpenResultset
        leenumeroSEGURO = resultados(0)
        Else
        leenumeroSEGURO = ""
    End If
    csql.Close
...
'leer
 ...
 MsgBox "COMPROBANTE PA N°" & numerocontable & " GENERADO EXITASAMENTE ", vbInformation, "ATENCION"
no:
frmcheque.Visible = False
dato4.text = ""
End Sub
