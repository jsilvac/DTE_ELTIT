...
Rem DATOS DE LA COLUMNA
    ...
    FORMATOGRILLA(1, 11) = "ANT"
    FORMATOGRILLA(1, 12) = "ANT"
...
'        csql.sql = csql.sql + " order by mc.rut,mc.fecha "
        ...
        csql.sql = csql.sql & "SELECT 'BH',mc.numero,mc.fecha,mc.liquido,mc.rut,autorizacancelacion,fechaautorizacionpago "
        csql.sql = csql.sql + "FROM boletasdehonorarios as mc "
         csql.sql = csql.sql + " where   mc.fecha between '" + fecha1 + "' AND '" + fecha2 + "' and autorizacancelacion='1' "
         
'        'para cargar las polizas de la esta
'        csql.sql = csql.sql & "union "
'        csql.sql = csql.sql & "SELECT 'PS',mc.numero,mc.fecha,mc.monto,mc.rutproveedor as rut,'0',fechavencimiento "
'        csql.sql = csql.sql + "FROM movimientoscontables  as mc "
'        csql.sql = csql.sql + " where   mc.fecha between '" + fecha1 + "' AND '" + fecha2 + "' and fechavencimiento='0000-00-00' and codigocuenta='11130001' "
'
         
         
        csql.sql = csql.sql + " order by rut,fecha "
        ...
        fechasum = Format(fechasistema, "yyyy") + "/" + Format(fechasistema, "mm") + "/" + Format(fechasistema, "dd")
         ...
         While Not resultados.EOF
            ...
            If resultados(0) = "BH" Then tipo = "BH"
            
            
            If tipo = "BH" Then
                montofacturas = leerabonofactura(resultados(0), tipo, resultados(1), resultados(4), cuentahonorarios, "D", resultados(2))
            Else
                montofacturas = leerabonofactura(resultados(0), tipo, resultados(1), resultados(4), CUENTAPROVEEDOR, "D", resultados(2))
            ...
            If montofacturas = 0 Then
             ...
             End If
             If resultados(0) <> "BH" And resultados(0) <> "PS" Then
                Grid1.Cell(LINEA, 4).text = leerdatos(contadb, "cuentascorrientes", "nombre", "rut='" + rutreal + "' and tipo='" + CUENTAPROVEEDOR + "' ")
             End If
               If resultados(0) = "PS" Then
                Grid1.Cell(LINEA, 4).text = leerdatos(contadb, "cuentascorrientes", "nombre", "rut='" + rutreal + "' and tipo='47150022' ")
             End If
             
             Grid1.Cell(LINEA, 5).text = resultados(2)
             Grid1.Cell(LINEA, 6).text = resultados(3)
...
'             Grid1.Cell(LINEA, 9).text = resultados(3) - OTROS
            ...
            End If
             ...
             Grid1.Cell(LINEA, 14).text = resultados(5)
                ...
                End If
                 If IsNull(resultados(6)) = True And resultados(5) = "1" Then
...
Public Sub modificaordenpago(numero, loc, pago, fecha, tipo, fechadocu, rutprove)
        ...
        If tipo <> "PS" Then
            tabla = "update  facturasdecompras  set  autorizacancelacion='" + pago + "',fechaautorizacionpago='" + Format(fecha, "yyyy-mm-dd") + "' "
            tabla = tabla & "WHERE numero= '" & numero & "' and rut='" & Replace(rutprove, "-", "") & "' and tipo='" & tipo & "' "
            sql.sql = tabla
            sql.Execute
       Else
             tabla = "update  movimientoscontables   set  fechavencimiento='" + Format(fecha, "yyyy-mm-dd") + "' "
            tabla = tabla & "WHERE numero= '" & numero & "' and rutproveedor='" & Replace(rutprove, "-", "") & "' and tipo='" & tipo & "' and fecha='" & Format(fechadocu, "yyyy-mm-dd") & "' "
            sql.sql = tabla
            sql.Execute
       End If
...
Sub GRABARCOMPROBANTE(ORDEN, montocheque, DIFERENCIA, fechacheque, TIPOPAGO, glosadiferencia, rutprove, fechado)
    ...
    Dim rut2 As String
        ...
        If csql.RowsAffected > 0 Then
            ...
            Else
                Call grabarcomprobante_lineas(DOCUMENTOPAGO, numerocontable, lineacontable, fecha, cuentahonorarios, " ", rut2, " ", "CANCELA DOCUMENTO", tipodocumento, numerodocumento, fechadocumento, fechavencimiento, monto, DH, USUARIOSISTEMA, MES, año, Format(Date, "yyyy-mm-dd"), Time, rutreal)
...
Dim DH As String
    ...
    Rem graba cheque
        ...
        If tipocontable = "PA" Then
        fecha = Format(fechasistema, "yyyy-mm-dd")
        Call grabacheque(CUENTABANCO, numerodocumento, fecha, monto, fechavencimiento, "NG", numerocontable, NOMBREGIRADO, "0")
        End If
End Sub
