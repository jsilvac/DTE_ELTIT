...
Begin VB.Form ControlPagoexterno01 
   BackColor       =   &H00FFC0C0&
   ...
   End
End
Attribute VB_Name = "ControlPagoexterno01"
...
Private Sub Command1_Click()
    ...
    For k = 0 To ComboLOCAL.ListCount - 1
       codigoempresa = codigoempresa & " cp.local='" & Mid(ComboLOCAL.List(k), 1, 2) & "' "
       If k < ComboLOCAL.ListCount - 1 Then
            codigoempresa = codigoempresa & " or "
       Else
    ...
    Call CargaGrillaInforme(1, 8)
        ...
        Call LEERTRABAJADORES(codigoempresa, Format(COMBOMES.ListIndex + 1, "00"), COMBOAÑO.text)
 ...
 If Verifica_FORM29(COMBOAÑO.text & "-" & COMBOMES.ListIndex + 1 & "-01", empresaactiva) = False Then
        If lblBanco.Caption <> "" Then
            If empresaconsulta <> empresaactiva Then MsgBox "EL LISTADO ES DE OTRA EMPRESA " & empresaconsulta, vbCritical, "ATENCION": GoTo no:
                rut_trabajador = impresion.Cell(1, 1).text
            ...
            For k = 1 To impresion.Rows - 1
                If impresion.Cell(k, 7).text = "1" And Val(impresion.Cell(k, 4).text) > 0 Then
                
                 If rut_trabajador <> impresion.Cell(k, 1).text Then
                        If TOTALCheque <> 0 Then
                            Call grabarcheque(TOTALCheque)
                            TOTALCheque = 0
                        End If
                        dato4.text = CDbl(dato4.text) + 1
                        NCHEQUE = CDbl(dato4.text) - 1
                        tipocontable = "PA"
                        numerocontable = LEERFOLIOCE("PA")
                        lineacontable = 0
                 ...
                 End If
                    ...
                    CUENTABANCO = "23100029"
                    fechavencimiento = Format(fechasistema, "yyyy-mm-dd")
                    monto = CDbl(impresion.Cell(k, 4).text)
                    DH = "D"
                If verificasiexiste2(rutcontable, CUENTABANCO, FECHACONTABLE, tipocontable, "CANCELA EXTERNO ") = False Then
                    lineacontable = lineacontable + 1
                    Call grabarcomprobante_lineas(tipocontable, numerocontable, lineacontable, _
                    FECHACONTABLE, CUENTABANCO, " ", rutcontable, " ", "CANCELA EXTERNO " & NOMBREGIRADO, _
            ...
            End If
            rut_trabajador = impresion.Cell(k, 1).text
        Next k
...
End Sub
    ...
    Private Sub imprimir()
        ...
        impresion.PageSetup.PrintFixedRow = True
         ...
         impresion.Column(7).Width = 0
        impresion.Cell(0, 8).text = "FIRMA"
        impresion.Column(8).Width = 200
         
        For k = 1 To impresion.Rows - 1
            If impresion.Cell(k, 4).text = 0 Then
                impresion.RowHeight(k) = 0
            End If
            impresion.Cell(k, 8).Border(cellEdgeBottom) = cellThin
        Next k
        impresion.PrintPreview
        impresion.Column(7).Width = 66
...
Public Sub generaInformeLV(ByRef data As Adodc, ByRef impresion As Grid, ByVal tipo As String, ByVal detalle As Boolean, ByVal codLoc As String, ByVal fecha1 As String, ByVal fecha2 As String)
    ...
    impresion.Rows = 1
    impresion.AutoRedraw = False
    Call cargaCabeza("PAGO PERSONAL EXTERNO " & MonthName(Format(fechasistema, "mm")) & " DE " & Format(fechasistema, "yyyy"), Mid(ComboLOCAL, 1, 2), impresion)
    impresion.AutoRedraw = True
    impresion.Refresh
End Sub
...
Private Sub CargaGrillaInforme(ByVal row As Integer, ByVal col As Integer)
        ...
        FORMATOGRILLA(1, 3) = "TRABAJO"
        FORMATOGRILLA(1, 4) = "MONTO"
        FORMATOGRILLA(1, 5) = "RUT BOLETA"
        FORMATOGRILLA(1, 6) = "N BOLETA"
...
'    csql.sql = csql.sql & "mt.año='" & añoconsulta & "' AND codigotg='0011' "
...
Sub LEERTRABAJADORES(loc_consulta, mesconsulta, AÑOCONSULTA)
   Dim resultados As rdoResultset
    Dim csql As New rdoQuery
    Dim cod_contable As String
    cod_contable = empresaactiva
    Set csql.ActiveConnection = contadb
    csql.sql = "  SELECT mt.rut,mt.nombre,motivo,IFNULL(if(montopagado=0,montoapagar,montopagado),'0') AS monto,rutboleta,numeroboleta "
    csql.sql = csql.sql & "FROM " & clientesistema & "remu.mt_fijo_externos AS mt INNER JOIN "
    csql.sql = csql.sql & "" & clientesistema & "ventas.sv_controlpersonal_externo AS cp ON cp.rut=mt.rut    "
    csql.sql = csql.sql & "WHERE cp.mes='" & Format(fechasistema, "mm") & "' AND "
    csql.sql = csql.sql & "cp.año='" & Format(fechasistema, "yyyy") & "' and " & loc_consulta & " and autorizado='1' "
    csql.sql = csql.sql & "GROUP BY rut ORDER BY nombre "
    csql.Execute
    ...
    If csql.RowsAffected > 0 Then
        ...
        While Not resultados.EOF
            ...
            impresion.Cell(impresion.Rows - 1, 6).text = resultados(5)
...
Private Sub impresion_Click()
    ...
    If impresion.ActiveCell.col = 7 Then
        montoboleta = verificaboleta(impresion.Cell(impresion.ActiveCell.row, 5).text, Format(impresion.Cell(impresion.ActiveCell.row, 6).text, "0000000000"))
        If montoboleta <> impresion.Cell(impresion.ActiveCell.row, 4).text And montoboleta <> 0 Then
            MsgBox "BOLETA DE HONORARIO NO TIENE MISMO VALOR QUE LO AUTORIZADO, POR FAVOR REVISAR ANTES DE PAGAR", vbCritical, "ATENCION"
            impresion.Cell(impresion.ActiveCell.row, 7).text = 0
        End If
         If montoboleta = 0 Then
            MsgBox "BOLETA DE HONORARIO NO ESTA INGRESADA EN SISTEMA, POR FAVOR INGRESAR ANTES DE PAGAR ", vbCritical, "ATENCION"
            impresion.Cell(impresion.ActiveCell.row, 7).text = 0
         End If
          If montoboleta = impresion.Cell(impresion.ActiveCell.row, 4).text Then
            impresion.Cell(impresion.ActiveCell.row, 7).text = 1
         End If
...
'    End If
    ...
    If col <> NewCol And col = 6 Then
        impresion.Cell(row, 5).text = Format(impresion.Cell(row, 5).text, "0000000000")
        impresion.Cell(row, 6).text = Format(impresion.Cell(row, 6).text, "0000000000")
        montoboleta = verificaboleta(impresion.Cell(row, 5).text, Format(impresion.Cell(row, 6).text, "0000000000"))
        If montoboleta <> impresion.Cell(row, 4).text And montoboleta <> 0 Then
            MsgBox "BOLETA DE HONORARIO NO TIENE MISMO VALOR QUE LO AUTORIZADO, POR FAVOR REVISAR ANTES DE PAGAR", vbCritical, "ATENCION"
            impresion.Cell(row, 7).text = 0
        End If
         If montoboleta = 0 Then
            MsgBox "BOLETA DE HONORARIO NO ESTA INGRESADA EN SISTEMA, POR FAVOR INGRESAR ANTES DE PAGAR ", vbCritical, "ATENCION"
            impresion.Cell(row, 7).text = 0
         End If
          If montoboleta = impresion.Cell(row, 4).text Then
            impresion.Cell(row, 7).text = 1
         End If
         
    End If
End Sub
Function verificaboleta(ruttrabajador, numerodocumento) As Double
    Dim csql As New rdoQuery
    Dim resultados As rdoResultset
    Set csql.ActiveConnection = contadb
    csql.sql = "select liquido from boletasdehonorarios "
    csql.sql = csql.sql & "where numero='" & numerodocumento & "' and rut='" & Format(ruttrabajador, "0000000000") & "' "
    csql.sql = csql.sql & "and añocontable='" & Format(fechasistema, "yyyy") & "' and mescontable='" & Format(fechasistema, "mm") & "' "
    csql.Execute
    verificaboleta = 0
    If csql.RowsAffected > 0 Then
        Set resultados = csql.OpenResultset
        verificaboleta = resultados(0)
    End If
    csql.Close
    Set csql = Nothing
