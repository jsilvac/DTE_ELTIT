...
Else
    MsgBox ("ESTE ARCHIVO NO ES UN CSV")
End If
...
Dim EsCartola As Boolean
Dim IniciaHoja As Boolean
Dim TerminaHoja As Boolean
Dim FinCartola As Boolean

'CONTENIDO DE LA CARTOLA
Dim numerocartola As String
Dim fecha As String
Dim SUCURSAL As String
Dim descripcion As String
Dim NUMERODOC As String
Dim montocargo As String
Dim montoabono As String
Dim saldodiario As String
...
columna = Split(varipaso, ";")
If InStr(columna(0), "Cartola ") > 0 And EsCartola = False Then
    EsCartola = True
End If
 
If EsCartola = True And IniciaHoja = False And InStr(UCase(columna(0)), "FECHA") > 0 Then
    IniciaHoja = True
    TerminaHoja = False
End If
  

If InStr(LCase(columna(0)), "Resumen") > 0 Then
    FinCartola = True
    MsgBox "CARTOLA PROCESADA EXITOSAMENTE" & vbNewLine & "LINEAS PROCESADAS " & Format(lin, "###,###,##0")
End If
If InStr(columna(0), "Nº Cartola") > 0 Then
    numerocartola = Format(columna(1), "0000000000")
End If


    If IniciaHoja = True Then
        If IsDate(columna(0)) = True Then
        L = L + 1
            
            fecha = Format(columna(0), "yyyy-mm-dd")
            SUCURSAL = columna(2)
            descripcion = columna(4)
            NUMERODOC = columna(3)
            montocargo = Replace(Replace(columna(5), ",", ""), ".", "")
            montoabono = Replace(Replace(columna(6), ",", ""), ".", "")
            saldodiario = Replace(Replace(columna(7), ",", ""), ".", "")
            
            
'            If montocargo = "" Then
''            montocargo = Replace(Replace(columna(7), ",", ""), ".", "")
'             montocargo = 0
'            End If
            
                    
            If montocargo = "" Then montocargo = 0
            If montoabono = "" Then montoabono = 0
            If saldodiario = "" Then saldodiario = 0
            lin = lin + 1
         '   Stop
            Call GRABACARTOLA(dato1.text & dato2.text & dato3.text, lin, numerocartola, fecha, descripcion, NUMERODOC, montocargo, montoabono, saldodiario, SUCURSAL)
        End If
    End If
...
If EsCartola = False Then
    MsgBox ("ARCHIVO NO CORRESPONDE A CARTOLA BANCO ESTADO ")
    Exit Sub
End If
...
End Sub
Sub GRABACARTOLA(banco, LINEA, numerocartola, fecha, descripcion, NUMERODOC, montocargo, montoabono, saldo, SUCURSAL)
    Dim campos(100, 10)
    Dim ABONO As String
    Dim tac As String
    Dim monto As String
    ...
    campos(0, 1) = banco
    campos(1, 1) = numerocartola
    campos(2, 1) = tac
    campos(3, 1) = fecha
    campos(4, 1) = tipo
    campos(5, 1) = descripcion
    campos(6, 1) = NUMERODOC
    campos(7, 1) = monto
    campos(8, 1) = SUCURSAL
    campos(9, 1) = SUCURSAL
    campos(10, 1) = ""
...
If Grid1.Cell(k, 9).text <> "CONTABILIZADO" Then
    ...
    End If
     ...
     If LEERcuentacontable(Grid1.Cell(k, 0).text) = True Then
        ...
        Set resultados = csql.OpenResultset
         ...
         While Not resultados.EOF
             ...
             If Option1.Value = True And Check1.Value = 0 Then
                    ...
                    LINEA = Grid1.Rows - 1
                     Grid1.Cell(LINEA, 0).text = resultados(5)
                    ...
                    Grid1.Cell(LINEA, 13).text = resultados("comparable")
                     ...
                     If LEERcuentacontable(resultados(5)) = True Then
                              Grid1.Cell(LINEA, 14).text = banco_cuenta
                            ...
                            End If
                     If resultados(3) = "A" And resultados(4) = "" Then
                        Grid1.Cell(LINEA, 0).text = ""
                          Grid1.Cell(LINEA, 14).text = "11500200"
                          Grid1.Cell(LINEA, 15).text = leerNombreMayor(Grid1.Cell(LINEA, 14).text)
                          banco_dh = "A"
                            Call LEERcuentacontabilizada(Grid1.Cell(LINEA, 14).text, resultados(2), resultados(7), banco_dh)
                              Grid1.Cell(LINEA, 9).text = conta_glosa
             ...
             If InStr(resultados("glosa"), "CHEQUE") > 0 Then
            ...
            End If
             ...
             If Option1.Value = True And Check1.Value = 1 Then
                If LEERcuentacontable(resultados(5)) = True Then
                              Grid1.Cell(LINEA, 14).text = LEERcuentacontable(resultados(5))
                              Grid1.Cell(LINEA, 15).text = leerNombreMayor(Grid1.Cell(LINEA, 14).text)
...
Public Function LEERcuentacontable(codigo) As Boolean
    ...
    Dim csql As New rdoQuery
        ...
        Set csql.ActiveConnection = conta
            ...
            csql.sql = "select * from cartolasbancarias_codigoscontables where nombre='" + codigo + "' and codigocontable<>'00000000' "
    ...
    Set resultados = csql.OpenResultset
        ...
        If empresaactiva = "01" Then
            If resultados(3) = "11500110" And dato1.text & dato2.text & dato3.text = "11120002" Then
                banco_cuenta = "11500200"
                banco_glosa2 = "RENDICION DEPOSITO BCO ESTADO ARRIENDO"
            End If
        End If
        
         If empresaactiva = "29" Then
            If resultados(3) = "11500110" And dato1.text & dato2.text & dato3.text = "11120004" Then
                banco_cuenta = "11500200"
                banco_glosa2 = "RENDICION DEPOSITO BCO ESTADO ARRIENDO"
            End If
            If resultados(3) = "11500110" And dato1.text & dato2.text & dato3.text = "11120003" Then
                banco_cuenta = "11500201"
                banco_glosa2 = "RENDICION DEPOSITO BCO ESTADO SEGUROS"
            End If
        End If
