...
Begin VB.Form publi0002 
   ...
   Begin XPFrame.FrameXp cabeza 
      ...
      Begin VB.CheckBox Check1 
         BackColor       =   &H00FFC0C0&
         Caption         =   "Busqueda Por Folio SII"
         Height          =   255
         Left            =   3240
         TabIndex        =   64
         Top             =   600
...
If sqlconta.status = 0 Then
    If dato1.text = "2" Then
        dato13b.text = LeedatoOCproveedor("FV", dato2.text, CONFI_EMPRESAFAE, "ordencompra", "98")
        fechaoc.Value = LeedatoOCproveedor("FV", dato2.text, CONFI_EMPRESAFAE, "ifnull(fechaoc,'0000-00-00')", "98")
...
If apagar = True Then apagar = False: Unload Me
...
Sub carga()
    ...
    DATO21.text = Mid(sqlconta.response(16, 3), 1, 2)
    DATO22.text = Mid(sqlconta.response(16, 3), 3, 2)
    nombrecrcc.Caption = leerNOMBREcrcc(sqlconta.response(16, 3))
...
If command = "imprime" Then
    ...
    End If
    If dato1.text <> "1" Then
        If documentocreado("FV", "98", CONFI_EMPRESAFAE, dato2.text, dato5.text & "-" & dato4.text & "-" & dato3.text) = False Then
...
'               Call Cargarpdf("33", Format(txtfolio.text, "0000000000"), dato5.text & "-" & dato4.text & "-" & dato3.text, dato9.text & DV.Caption, 1)
                ...
                Call Cargarpdf("33", txtfolio.text, dato5.text & "-" & dato4.text & "-" & dato3.text, Val(dato9.text) & DV.Caption, 0)
                Call Sleep(10000)
                Call Cargarpdf("33", txtfolio.text, dato5.text & "-" & dato4.text & "-" & dato3.text, Val(dato9.text) & DV.Caption, 1)
...
Sub leefactura()
    ...
    dato2.text = numero_interno("33", dato2.text, CONFI_EMPRESAFAE)
    End If
    condicion = "tipo=" + "'" + dato1.text + "'" + " and numero=" + "'" + dato2.text + "'"
    ...
    If Check1.Value = 1 Then
    MsgBox "estado solo de consulta , enter para continuar"
    apagar = True
