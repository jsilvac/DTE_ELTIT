...
Option Explicit
    ...
    Private totalanticipo As Double
...
Public Sub grabarEspecialesguias(numero, LINEA, cuenta, glosa, monto, DH, TIPODO, numerodo, rut)
        ...
        campos(7, 1) = numerodo
        campos(8, 1) = rut
        campos(9, 1) = dato1.text
        
        campos(0, 2) = clientesistema & "conta" + empresaactiva + ".facturasdecompras_anexospago"
...
Sub grabarAnticipos()
        ...
        Dim tabla As String
'Set csql.ActiveConnection = contadb
rutpro = Mid(prove0009.Grid1.Cell(prove0009.Grid1.ActiveCell.row, 3).text, 1, 9) + Mid(prove0009.Grid1.Cell(prove0009.Grid1.ActiveCell.row, 3).text, 11, 1)
...
fecha2 = Format(fechasistema, "YYYY-mm-dd")
        ...
        csql.sql = "SELECT sm.debeanterior-sm.haberanterior,"
        csql.sql = csql.sql + "(select sum(if(mo.dh='D',monto,monto*-1)) from movimientoscontables as mo where fecha between '" + fecha1 + "' and '" + fecha2 + "' "
        csql.sql = csql.sql & "and sm.tipo=mo.codigocuenta and sm.rut=mo.rutctacte) as saldo "
        
        csql.sql = csql.sql + "FROM saldosctacte as sm "
        csql.sql = csql.sql + "WHERE año = '" & Format(fechasistema, "yyyy") & "' "
   
        csql.sql = csql.sql + "AND rut = '" & rutpro & "' and tipo='23100027' "
...
If csql.RowsAffected > 0 Then
    ...
    While Not resultados.EOF
        If resultados(0) + resultados(1) <> 0 Then
        dato1.text = "23100027"
         lblcuenta.Caption = leerdatos(contadb, "cuentasdelmayor", "nombre", "codigo='" + dato1.text + "' and año='" + año + "' ")
         dato2.text = "ANTICIPOS"
         dato3.text = resultados(0) + resultados(1)
         totalanticipo = resultados(0) + resultados(1)
         dato4.text = "H"
         DATO5.text = rutpro
        End If
'        LINEA = LINEA + 1
'
'        Call grabarEspecialespublicidad(numero, LINEA, "23100027", "ANTICIPO " & resultados(1) & " DEL " & Format(resultados(0), "dd-mm-yyyy"), resultados(2), "H", resultados(3), resultados(4), rutpro)
'
...
End Sub
    ...
    KeyAscii = esNumero(KeyAscii)
    If KeyAscii = 13 Then
        If dato1.text = "23100027" Then
            If totalanticipo >= dato3.text Then
                dato4.SetFocus
            Else
                MsgBox "NO PUEDE SUBIR EL VALOR DEL ANTICIPO. TOTAL ES DE  " & Format(totalanticipo, "$ ###,###,##0") & " ", vbCritical, "ATENCION"
                dato3.text = ""
                dato3.SetFocus
            End If
        Else
...
'=============================================================================
    Private Sub leerEspeciales()
        ...
        tabla = tabla & "WHERE numero= '" & numero.text & "' and tipo='" & Mid(tipo.text, 1, 1) & "' ORDER BY linea asc "
        sql.sql = tabla
        sql.Execute
...
'=============================================================================
    Public Sub grabarEspeciales(LINEA)
        ...
        campos(7, 1) = tipo.text
        
        campos(0, 2) = clientesistema + "conta" + empresaactiva + ".facturasdecompras_anexospagos"
...
Public Sub modificalineas(numero, LINEA, lineanueva)
        ...
        condicion = "numero='" + numero + "' and linea='" & LINEA & "' and tipo='" + tipo.text + "' "
        op = 3
        sqlconta.response = campos
        Set sqlconta.conexion = gestionrubro
        Call sqlconta.sqlconta(op, condicion)
...
'        campos(0, 2) = clientesistema + "gestion" + rubro + ".l_ordendecompra_anexopagos_" & localorden
        ...
        condicion = "numero = '" & numero.text & "' AND linea = '" & LINEA & "' and tipo='" + Mid(tipo.text, 1, 1) + "' "
