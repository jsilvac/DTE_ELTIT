...
Begin VB.Form contabilizapromotora 
   Appearance      =   0  'Flat
   BackColor       =   &H00FFC0C0&
   ...
   End
End
Attribute VB_Name = "contabilizapromotora"
...
If Verifica_Permiso(ingreso01.Caption, "elimina") = True Then
            ...
            Else
                Call eliminacomprobantesmasivos("CP", MES, año, empresaactiva)
            End If
...
Dim resultados As rdoResultset
    ...
    fecha2 = año + "-" + MES + "-" + "31"
        ...
        csql.sql = csql.sql + "from " + clientesistema + "ventas.sv_cuotas_detalle "
        csql.sql = csql.sql + "where fechacompra>='" + fecha1 + "' AND fechacompra<='" + fecha2 + "' and numerocuota='1' group by fechacompra order by fechacompra "
        Else
        csql.sql = "SELECT fecha,sum(montocuotas),sum(interesesmora),sum(montocuotas+interesesmora) "
        csql.sql = csql.sql + "from " + clientesistema + "ventas.sv_cuotas_pago_cabeza "
        ...
        Set resultados = csql.OpenResultset
         ...
         While Not resultados.EOF
            ...
            If Option1.Value = True Then
            If LEEDOCUMENTO("11200027", resultados(0), "D", "CV") = True Then
             Grid1.Cell(LINEA, 6).text = "1"
            End If
            End If
            
            If Option1.Value = False Then
            If LEEDOCUMENTO("11200027", resultados(0), "H", "CP") = True Then
             Grid1.Cell(LINEA, 6).text = "1"
            End If
            End If
...
Dim resultados As rdoResultset
    ...
    lineascomprobante = 0
    numero = LEERULTIMOFOLIO("CV")
        INTERESES = 0
        Set csql.ActiveConnection = contadb
        csql.sql = "SELECT montocuota,cantidadcuotas,rut,montocuota*cantidadcuotas,local,montocredito,tipo,numero "
        csql.sql = csql.sql + "from " + clientesistema + "ventas.sv_cuotas_detalle "
        csql.sql = csql.sql + "where fechacompra>='" + fecha1 + "' AND fechacompra<='" + fecha2 + "' and numerocuota='1'   order by rut "
        ...
        Set resultados = csql.OpenResultset
         ...
         While Not resultados.EOF
                ...
                CRCC = ""
                monto = resultados(3)
                glosa = "CARGA CUENTA  " & resultados(1) & " x " & Format(resultados(0), "###,###") & " x compra " + leerdatoslocal(resultados(4), "nombre")
                Call existerut(Format(fechasistema, "yyyy"), cuenta, resultados(2))
...
csql.Close
        ...
        csql2.sql = csql2.sql + "from " + clientesistema + "ventas.sv_cuotas_detalle "
        csql2.sql = csql2.sql + "where fechacompra>='" + fecha1 + "' AND fechacompra<='" + fecha2 + "' and numerocuota='1' group by local,tipo order by local "
    ...
    numerorut = ""
    monto = resultados(0)
    glosa = "CARGA REPACTACIONES " + leerdatoslocal(resultados(1), "nombre")
    End If
    ...
    monto = resultados(0)
    glosa = "CARGA DEUDA POR VENTAS " + leerdatoslocal(resultados(1), "nombre")
    End If
    ...
    numerorut = ""
    monto = resultados(0)
    glosa = "CARGA COBROS POR SEGURO " + leerdatoslocal(resultados(1), "nombre")
    End If
    ...
    Call grabarcomprobante_lineas(TIPODO, numero, LINEA, fecha, cuenta, cuenta, numerorut, CRCC, glosa, "CV", numero, fecha, fecha, monto, "H", USUARIOSISTEMA, Format(fecha, "MM"), Format(fecha, "yyyy"), Format(fechasistema, "yyyy-mm-dd"), Time, "")
...
Rem GRABA LINEA INTERES
    ...
    numerorut = ""
    monto = INTERESES
    glosa = "INTERES POR COMPRA  "
    Call grabarcomprobante_lineas(TIPODO, numero, LINEA, fecha, cuenta, "", "", CRCC, glosa, "CV", numero, fecha, fecha, monto, "H", USUARIOSISTEMA, Format(fecha, "MM"), Format(fecha, "yyyy"), Format(fechasistema, "yyyy-mm-dd"), Time, "")
...
Dim resultados As rdoResultset
    ...
    CRCC = ""
    monto = resultados(2)
    glosa = "CANCELA CUOTAS  "
    sumador(resultados(5)) = sumador(resultados(5)) + (resultados(2) + resultados(3))
    Call existerut(Format(fechasistema, "yyyy"), cuenta, resultados(1))
    Call grabarcomprobante_lineas("CP", numero, LINEA, fecha, cuenta, cuenta, resultados(1), CRCC, glosa, "PA", resultados(4), fecha, fecha, monto, "H", USUARIOSISTEMA, Format(fecha, "MM"), Format(fecha, "yyyy"), Format(fechasistema, "yyyy-mm-dd"), Time, "")
    INTERESES = INTERESES + resultados(3)
...
csql.Close
  ...
  If sumador(k) <> 0 Then
    ...
    monto = sumador(k)
    glosa = "CARGA PAGOS CLIENTES "
    Call grabarcomprobante_lineas("CP", numero, LINEA, fecha, cuenta, "", numerorut, "", glosa, "CV", numero, fecha, fecha, monto, "D", USUARIOSISTEMA, Format(fecha, "MM"), Format(fecha, "yyyy"), Format(fechasistema, "yyyy-mm-dd"), Time, "")
...
Rem GRABA LINEA INTERES
    ...
    numerorut = ""
    monto = INTERESES
    glosa = "INTERES POR MORA  "
    Call grabarcomprobante_lineas("CP", numero, LINEA, fecha, cuenta, "", "", CRCC, glosa, "CV", numero, fecha, fecha, monto, "H", USUARIOSISTEMA, Format(fecha, "MM"), Format(fecha, "yyyy"), Format(fechasistema, "yyyy-mm-dd"), Time, "")
